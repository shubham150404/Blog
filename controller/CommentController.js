...
exports.Comment_create = async function (req, res, next) {
  try {
    ...
    const Jwt_Comment = jwt.sign({ id: Comment_data._id },process.env.SECRET_BLOG)
    res.status(201).json({
      status: "sucess",
...
exports.Comment_get = async function (req, res, next) {
  try {
    const Comment_get = await Comment.find()
...
exports.Comment_Update = async function (req, res, next) {
  try {
    ...
    const Comment_get = await Comment.findByIdAndUpdate(id, req.body)
    res.status(201).json({
      status: "sucess",
...
exports.Comment_sequre = async function (req, res, next) {
  try {
    ...
    const Jwt_token = jwt.verify(Comment_Token,process.env.SECRET_BLOG);
    console.table(Jwt_token)
    next()
  } catch (error) {
    res.status(404).json({
      status: "faild",
      message: "Comment is not sequre",
    })
  }
}